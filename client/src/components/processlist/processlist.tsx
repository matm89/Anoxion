...
export const ProcessList = memo(function ProcessListComponent({
  processList,
  processes,
}: ProcessProps) {
  ...
  function getResult(proc: string) {
    const procs = processes.flat().filter((p) => p.process_id == proc);
  ...
  return (
    <div className="flex flex-col w-full items-center">
            ...
            <div
              ...
              <span
                className={`text-lg font-semibold text-blue-900
                 ${getResult(proc as string) == 'Failure' ? 'text-red-500' : 'text-green-500'}`}>
              ...
              <button
                ...
                className="mt-3 relative px-4 py-2 text-sm font-medium text-white rounded-lg group bg-gradient-to-br from-green-400 to-blue-600 hover:from-green-500 hover:to-blue-700 transition-all duration-150 focus:ring-2 focus:ring-green-300">
      ...
      {/* === PROCESS DETAILS ROW === */}
      <div id="processDetails" className="w-full mt-6 flex justify-center">
        {selectedProcess !== null && (
          <div className="w-full max-w-6xl">
            <ProcessDetails
              processes={processes.flat().filter((p) => p.process_id === selectedProcess)}
            />
          </div>
        )}
      </div>
